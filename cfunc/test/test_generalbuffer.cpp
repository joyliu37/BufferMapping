...
int main(int argc, char* argv[]) {
    ...
    vector<vector<int> > read_stream(961, vector<int>(9));
...
void ReadWriteBlockCheck(const vector<int> & data,
    ...
    //hard code the row size into test bench, we need to read one more stencil out,
    //after finish write to buffer
    for (int i = 0; i < buffer->getWriteIteration() + 31; i ++) {
        vector<int> in_data(buffer->getInPort(), 0);
        if (i < buffer->getWriteIteration() ) {
