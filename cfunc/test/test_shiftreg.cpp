...
int main(int argc, char* argv[]) {
    ...
    vector<vector<int> > read_stream(16128, vector<int>(2));
    ...
    cout << "Test passed for shif tregister!\n";
        ...
        for (int x = 0; x < 63; x ++) {
            for (int ky = 0; ky < 1; ky ++) {
                for (int kx = 0; kx < 2; kx ++) {
                    int addr = (y + ky)* 64 + x + kx;
                        gold_stream[cnt][kx + ky*1] = addr;
                }
            }
            cnt ++;
        }
    ...
    for (int i = 0; i < buffer->getWriteIteration(); i ++) {
        ...
        if (out_valid){
            for (int port = 0; port < buffer->getOutPort(); port ++){
                //cout << "outdata: " << out_data[port] << ", ref data: " << data[read_stream[output_cnt][port]] << endl;
